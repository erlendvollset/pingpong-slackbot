...
from pingpong.slackbot import BotCommand, CommandType
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            BotCommand(CommandType.MATCH, "<@KD839FK38> <@9FJ48GJF8> 11 0", CHANNEL_ID, USER_ID, PINGPONG_BOT_ID),
        ),
        (
            text_to_slack_event("<@OTHER1234> match <@KD839FK38> <@9FJ48GJF8> 11 0"),
            BotCommand(CommandType.MATCH, "<@KD839FK38> <@9FJ48GJF8> 11 0", CHANNEL_ID, USER_ID, "OTHER1234"),
        ),
        (
            text_to_slack_event(f"<@{PINGPONG_BOT_ID}> stats"),
            BotCommand(CommandType.STATS, None, CHANNEL_ID, USER_ID, PINGPONG_BOT_ID),
        ),
        (
            text_to_slack_event(f"nomention"),
            BotCommand(None, None, CHANNEL_ID, USER_ID, None),
        ),
        (
            text_to_slack_event(f"<@{PINGPONG_BOT_ID}> random-invalid-command"),
            BotCommand(None, None, CHANNEL_ID, USER_ID, PINGPONG_BOT_ID),
        ),
    ],
)
def test_bot_command_from_slack_event(test_input: dict, expected: BotCommand) -> None:
    assert BotCommand.from_slack_event(test_input) == expected
